(function () {
  ...
  function injectButtons() {
    ...
    for (var i = 0; i < images.length; i++) {
      ...
      result.style.cssText = 'margin:2px 0;padding:3px 5px;font-size:10px;line-height:1.3;background:#f0f4f8;border-radius:2px;display:none;white-space:normal;max-width:280px;';
      ...
      btn.addEventListener('click', async function () {
        ...
        try {
          ...
          result.innerHTML = '<b>en:</b> ' + escapeHtml(alt.en) + '<br><b>no:</b> ' + escapeHtml(alt.no) + cachedTag;
        } catch (e) {
          result.textContent = 'Error: ' + e.message;
        } finally {
          btn.disabled = false;
          btn.textContent = 'Lennard er best!';
        }
      });
